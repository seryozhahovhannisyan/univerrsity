...
function group_subject() {
    ...
    $(".checkbox_group").live('change', function () {
        ...
        if (!$(this).is(':checked')) {
            ...
            $.ajax({
                url: "select_group_subjects_users.htm",
                type: "post",
                dataType: "json",
                async: false,
                data: {
                ...
                success: function (data) {
                    ...
                    if (data != null && data.responseDto.status == "SUCCESS") {
                        ...
                        $new_td_users = $parent_row.find(".append_data_user");
                        ...
                        $.each(users, function (i, item) {
                            $new_user_data = $(
                                    '<tr>' +
                                            '<td><input type="checkbox"  checked name="userIdes" value="' + item.id + '"></td>' +
                                            '<td>' + item.name + '</td>' +
                                            '</tr>'

                            );
                            $table_users.append($new_user_data);
                        });
...
function add_topic_form() {
    $(".add_topic_form_button").click(function () {
        ...
        var userIdes = $("input[name='userIdes']");
        $form = $(".add_topic_form");
        $form.append(groupSubjectIdes);
        $form.append(userIdes);
        $form.submit();
    });
}
    ...
    <div id="universities_block" class="sblock" style="float: left;
        ...
        <div class="universities_content">
            <table style="width: 100%">
                ...
                <form class="add_topic_form" action="add_topic.htm" method="post" enctype="multipart/form-data">
                ...
                <%-- <s:form action="add_topic" method="post" enctype="multipart/form-data">
